...
import { 
  ...
  Compass, BarChartHorizontal, CalendarRange, Signal, BarChart3, 
...
const App = () => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const parsed = dataRows.map((row, i) => {
          ...
          const marketVal = (row[idx.market] || 'Uncategorized').trim();
  ...
  useEffect(() => {
    ...
    const monthFiltered = selectedMonth === 'All' ? yearFiltered : yearFiltered.filter(t => t.month === selectedMonth);
    const catFiltered = selectedCategory === 'All' ? monthFiltered : monthFiltered.filter(t => t.category === selectedCategory);
  ...
  }, [selectedYear, selectedMonth, selectedCategory, rawTrades, datePreset]);
  ...
  const processedData = useMemo(() => {
    ...
    const yearData = {}, monthData = {}, dayData = {}; 
    ...
    const generateBrief = (wr, rr) => {
        let type = "Standard Participant";
        if (parseFloat(wr) > 60 && parseFloat(rr) > 1.2) type = "Institutional Sculptor";
        else if (parseFloat(rr) > 2) type = "Momentum Specialist";
        else if (ruleAlignedCount / tradeTotalCount > 0.8) type = "Disciplined Operator";
        const topMistake = errorStats.length > 0 ? errorStats[0].cat : "Early entry";
        const topEmotion = eStats.sort((a,b) => b.absImpact - a.absImpact).length > 0 ? eStats.sort((a,b) => b.absImpact - a.absImpact)[0].name : "Fear";
        const prioritySteps = [
          parseFloat(rr) < 1.2 ? `Optimize R:R immediately to 1:1.5+ range.` : `Scale capital on 'A' setups with 1:${rr} efficiency.`,
          `Stop the ${topMistake} leak. This is your primary capital drain.`,
          `Enforce a 15-min pause when feeling ${topEmotion}.`
        ];
        return {
            profile: type, strength: parseFloat(wr) > 50 ? "Accuracy" : "Risk Mgt",
            weakness: (Math.abs(revengeLoss) / (winTotal || 1)) > 0.15 ? "Self-Control" : "Sample Size",
            narrative: `Institutional analysis suggests a ${type} profile. You capture ${formatCurrency(avgWin)} per win while risking ${formatCurrency(avgLoss)}.`,
            reviewStatement: parseFloat(rr) > 1.5 ? "Excellent capital preservation. Profit engine is structurally larger than risk." : "System stability is moderate. Focus on increasing average win distance.",
            prioritySteps
        };
    };

    return {
      metrics: { net: cumulativePL, winRate: winRateValue, total: tradeTotalCount, avgWin, avgLoss, overallRR: overallRRValue, maxProfit: Math.max(...filtered.map(t => t.pl > 0 ? t.pl : 0), 0), maxLoss: Math.max(...filtered.map(t => t.pl < 0 ? Math.abs(t.pl) : 0), 0), pf: (winTotal / (lossTotal || 1)).toFixed(2), expectancy: (cumulativePL / tradeTotalCount).toFixed(0), maxLosingStreak, maxWinningStreak, peakDD: maxDDValue, profitDD: (cumulativePL / (Math.abs(maxDDValue) || 1)).toFixed(2), mindsetEfficiency: Math.round(((winTotal - Math.abs(revengeLoss)) / (winTotal || 1)) * 100), maxTradesInDay: Math.max(...Object.values(dayData).map(d => d.count), 0), avgTradesPerActiveDay: Object.keys(dayData).length > 0 ? (tradeTotalCount / Object.keys(dayData).length).toFixed(1) : 0 },
      ...
      hierarchical: { yearData, monthData, dayData },
      barData: selectedYear === 'All' ? Object.entries(yearData).map(([n, d]) => ({ name: String(n), pl: d.pl })) : selectedMonth === 'All' ? Object.entries(monthData).map(([k, d]) => ({ name: String(k.split(' ')[0]), pl: d.pl })) : Object.entries(dayData).map(([d, val]) => ({ name: `D${String(d)}`, pl: val.pl })),
      maxDayAbsVal: Math.max(...Object.values(dayData).map(d => Math.abs(d.pl)), 1),
  ...
  const { metrics = {}, scores = {}, hierarchical = {}, barData = [], maxDayAbsVal = 1, bestPeriod = ['', {}], worstPeriod = ['', {}], statusStats = [], equity = [], outcomeDist = [], sizingData = [], qualityStats = [], setupAnalysis = [], emotionStats = [], errors = [], dynamicAudit = { start: [], continue: [] }, learnings = [], weekdayEdge = [], bestDay, worstDay, aiBrief = {} } = processedData || {};
                ...
                <div className="flex flex-wrap items-center justify-center md:justify-end gap-2">
                  ...
                  <div className="flex items-center gap-2 bg-slate-900 border border-slate-800 p-1.5 rounded-xl">
                    ...
                    <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="bg-slate-900 text-slate-100 text-[10px] font-black uppercase outline-none cursor-pointer pr-2 [color-scheme:dark] border-none">
                      ...
                      <option className="bg-slate-900" value="Crypto">Crypto Assets</option>
                    </select>
                  </div>
                  <div className="flex items-center gap-2 bg-slate-900 border border-slate-800 p-1.5 rounded-xl">
            ...
            {activeTab === 'performance' && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <MetricCard title="Net P&L" value={formatCurrency(metrics.net || 0)} icon={DollarSign} trend={(metrics.net || 0) >= 0 ? "up" : "down"} size="large" />
                ...
                <div className="mb-10">
                  {selectedYear === 'All' && datePreset === 'All' ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-7 gap-2 animate-in fade-in duration-500">
                        ...
                        {(() => {
                           const baseDate = rawTrades[0]?.jsDate || new Date();
                           const targetYear = selectedYear === 'All' ? baseDate.getUTCFullYear() : parseInt(selectedYear);
                           const targetMonth = selectedMonth === 'All' ? baseDate.getUTCMonth() : MONTH_MAP[selectedMonth];
                           const firstDayOfMonth = new Date(Date.UTC(targetYear, targetMonth, 1)).getUTCDay();
                           return [
                             ...Array.from({ length: firstDayOfMonth }).map((_, i) => <div key={`empty-${i}`} className="h-24 bg-transparent" />),
                             ...Array.from({ length: 31 }).map((_, i) => {
                               const dayNum = i + 1; const dayVal = (hierarchical.dayData || {})[dayNum] || { pl: 0, count: 0 };
