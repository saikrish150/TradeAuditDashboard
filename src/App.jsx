...
const App = () => {
  ...
  const processedData = useMemo(() => {
    ...
    const getPL = (t) => {
      // If there's already a non-zero pl number, use it
      if (t.pl !== undefined && t.pl !== '' && parseFloat(t.pl) !== 0) return parseFloat(t.pl) || 0;
      
      // If pl is 0 or missing, hunt in originalData
      if (t.originalData) {
    ...
    return {
      ...
      snapshots: rawSnapshots.filter(s => {
        ...
        if (datePreset === 'CurrentMonth') {
          ...
          if (selectedYear !== 'All' && sDate.getFullYear().toString() !== selectedYear) return false;
          if (selectedMonth !== 'All' && sDate.toLocaleString('default', { month: 'long' }) !== selectedMonth) return false;
        }
        return true;
      }),
      filteredNotes: notes.filter(n => {
        ...
        if (datePreset === 'CurrentMonth') {
          ...
          if (selectedYear !== 'All' && nDate.getFullYear().toString() !== selectedYear) return false;
          if (selectedMonth !== 'All' && nDate.toLocaleString('default', { month: 'long' }) !== selectedMonth) return false;
        }
        return true;
      })
    };
