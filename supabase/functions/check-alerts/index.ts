...
serve(async (req) => {
  ...
  try {
    // 🛑 IMPORTANT: If your tokens were hardcoded, put them here instead of using Deno.env.get()!
    const TELEGRAM_BOT_TOKEN = Deno.env.get('TELEGRAM_BOT_TOKEN') || "YOUR_BOT_TOKEN_HERE"
    const TELEGRAM_CHAT_ID = Deno.env.get('TELEGRAM_CHAT_ID') || "YOUR_CHAT_ID_HERE"
